...
public class JobService {
    ...
    /**
     * Elimina un oggetto dal database. Quando si elimina un incarico, allora vengono eliminati anche:
     * <ul>
    ...
    public void deleteJob(final Job job) {
        ...
        Collection<Operation> operationsJob = operationService.getOperationsJob(job);
    ...
    public String getNextProtocol() {
        ...
        /*
         * costruisco il nuovo protocollo, se è usato l'anno allora lo elimino dal calcolo
         */
        ...
        protocol =
                GreenToneUtilities.leftPadding(protocol, ConfigurationProperties.PROTOCOL_PADDING_CHAR,
                        ConfigurationProperties.JOB_PROTOCOL_NUMERIC_LENGTH);
        if (yearsEnabled) {
            int year = Calendar.getInstance().get(Calendar.YEAR);
            protocol = year + ConfigurationProperties.JOB_PROTOCOL_SEPARATOR + protocol;
        }
        return protocol;
    }
    ...
    public void addYearToAllJobProtocols() {
        ...
        SortedList<Job> sortedJobs = new SortedList<Job>(getAllJobs(), new Comparator<Job>() {
            @Override
        ...
        /*
         * Considera il caso [0001,0002] con 0001 iniziato nel 2011 e 0002 nel 2012 che deve diventare
         * [2011-0001,2012-0001]
         */
        int year = 0;
        int counter = 1;
     ...
     *             eccezione in caso in cui si venga a superare il limite di incarichi gestibili
     */
    public void removeYearToAllJobProtocols() throws IndexOutOfBoundsException {
        ...
        int limit = (int) Math.pow(10d, new Integer(ConfigurationProperties.JOB_PROTOCOL_NUMERIC_LENGTH).doubleValue());
        ...
        /*
         * rinumero tutti gli incarichi in questo modo il caso [2011-0001, 2012-0001] in [0001,0002]
         */
    ...
    /**
     * Restituisce la lista degli incarichi scaduti, per i quali non è ancora stata impostata una data di fine.
     * 
     * @return la lista degli incarichi scaduti, per i quali non è ancora stata impostata una data di fine
     */
    public Collection<Job> getExpiredJobs() {
        FilterList<Job> expiredJobs = new FilterList<Job>(allJobs, new Matcher<Job>() {
            @Override
            public boolean matches(Job arg0) {
                if (arg0.getDueDate() != null && arg0.getFinishDate() == null && arg0.getDueDate().isBeforeNow()) {
                    return true;
                }
                return false;
            }
    ...
    /**
     * Restituisce la lista degli incarichi in scadenza nei prossimi giorni passati in ingresso.
     * 
     * @param daysInterval
     * @return la lista degli incarichi in scadenza nei prossimi giorni passati in ingresso
     */
    public Collection<Job> getNextExpiringJobs(int daysInterval) {
        final DateTime trigger = new DateTime().plusDays(daysInterval);

        FilterList<Job> nextExpiringJobs = new FilterList<Job>(allJobs, new Matcher<Job>() {
            @Override
            public boolean matches(Job arg0) {
                if (arg0.getDueDate() != null && arg0.getFinishDate() == null
                        && arg0.getDueDate().isBefore(trigger.getMillis()) && !arg0.getDueDate().isBeforeNow()) {
                    return true;
                }
                return false;
            }
    ...
    /**
     * Restituisce la lista degli incarichi che nella descrizione contengono la stringa passata come parametro.
     * 
     * @param str
     *            la stringa da cercare nelle descrizioni degli incarichi
     * @return la lista degli incarichi che nella descrizione contengono la stringa passata come parametro
     */
    public Collection<Job> getJobsContainingDescription(final String str) {
        FilterList<Job> filterList = new FilterList<Job>(allJobs, new Matcher<Job>() {
