...
public class OptionsPanel extends ContextualPanel<Void> {
    ...
    protected JPanel createHeaderPanel() {
        ...
        JLabel titleLabel = new JLabel(getResourceMap().getString("viewOptions.Panel.title"));
        titleLabel.setFont(FontProvider.TITLE_SMALL);
        titleLabel.setIcon(getResourceMap().getIcon("viewOptions.Panel.titleIcon"));
    ...
    private JPanel getSystemPanel() {
        if (systemPanel == null) {
            ...
            systemPanel.setBorder(BorderFactory.createTitledBorder(getResourceMap().getString(
                    "viewOptions.Panel.systemTitle")));
    ...
    /**
     * Restituisce il flag di abilitazione del richiesta di chiusura dell'applicazione.
     * 
     * @return il flag di abilitazione del richiesta di chiusura dell'applicazione
     */
    ...
    private JPanel getAppPanel() {
        if (appPanel == null) {
            appPanel = new JPanel(new MigLayout("", "[][10%][][10%][][10%][][10%]", "[][][][][30%][][][]"));
            appPanel.setBorder(BorderFactory.createTitledBorder(getResourceMap()
                    .getString("viewOptions.Panel.appTitle")));
            JLabel vacazioneLabel = new JLabel(getResourceMap().getString("viewOptions.Panel.vacazione"));
            JLabel vacazioneAiutanteLabel =
                    new JLabel(getResourceMap().getString("viewOptions.Panel.vacazioneAiutante"));
            JLabel useYearInJobsProtocolLabel =
                    new JLabel(getResourceMap().getString("viewOptions.Panel.useYearInJobsProtocol"));
            JLabel nameLabel = new JLabel(getResourceMap().getString("viewOptions.Panel.officeName"));
            JLabel addressLabel = new JLabel(getResourceMap().getString("viewOptions.Panel.officeAddress"));
            JLabel cityLabel = new JLabel(getResourceMap().getString("viewOptions.Panel.officeCity"));
            JLabel provinceLabel = new JLabel(getResourceMap().getString("viewOptions.Panel.officeProvince"));
            JLabel capLabel = new JLabel(getResourceMap().getString("viewOptions.Panel.officeCAP"));
            JLabel pivaLabel = new JLabel(getResourceMap().getString("viewOptions.Panel.officePIVA"));
            JLabel cfLabel = new JLabel(getResourceMap().getString("viewOptions.Panel.officeCF"));
            JLabel tel1Label = new JLabel(getResourceMap().getString("viewOptions.Panel.officeTel1"));
            JLabel tel2Label = new JLabel(getResourceMap().getString("viewOptions.Panel.officeTel2"));
            JLabel faxLabel = new JLabel(getResourceMap().getString("viewOptions.Panel.officeFax"));
            JLabel emailLabel = new JLabel(getResourceMap().getString("viewOptions.Panel.officeEmail"));
            JLabel logoLabel = new JLabel(getResourceMap().getString("viewOptions.Panel.officeLogo"));
    ...
    public JButton getLogoButton() {
        if (logoButton == null) {
            logoButton = new JButton(getResourceMap().getString("viewOptions.Panel.open"));
    ...
    public JFormattedTextField getVacazioneAiutanteTextField() {
        if (vacazioneAiutanteTextField == null) {
            ...
            vacazioneAiutanteTextField.addFocusListener(new FocusAdapter() {
                @Override
                public void focusLost(FocusEvent e) {
                    vacazioneAiutanteTextField.setText(vacazioneAiutanteTextField.getText().replace('.', ','));
                }
            });
        }
    ...
    /**
     * Restituisce il flag per l'uso dell'anno nella composizione del protocollo di un incarico.
     * 
     * @return il flag per l'uso dell'anno nella composizione del protocollo di un incarico
     */
                ...
                public void actionPerformed(ActionEvent arg0) {
                    ...
                    if (result)
                        JOptionPane.showMessageDialog(getParent(),
                                getResourceMap().getString("viewOptions.Panel.logsDeleted"), getResourceMap()
                                        .getString("Application.title"), JOptionPane.INFORMATION_MESSAGE);
                    else
                        JOptionPane.showMessageDialog(getParent(),
                                getResourceMap().getString("viewOptions.Panel.logsNotDeleted"), getResourceMap()
                                        .getString("Application.title"), JOptionPane.WARNING_MESSAGE);
                }
            });
            deleteLogsButton.setIcon(getResourceMap().getIcon("viewOptions.Panel.deleteLogsIcon"));
            deleteLogsButton.setText(getResourceMap().getString("viewOptions.Panel.deleteLogsText"));
        }
    ...
    public JButton getDeleteLogoButton() {
        if (deleteLogoButton == null) {
            ...
            deleteLogoButton.setIcon(getResourceMap().getIcon("viewOptions.Panel.deleteLogsIcon"));
            deleteLogoButton.setText(getResourceMap().getString("viewOptions.Panel.deleteLogsText"));
        }
    ...
    public JComboBox getLogLevelComboBox() {
        if (logLevelComboBox == null) {
            logLevelComboBox =
                    new JComboBox(new Level[] { Level.OFF, Level.SEVERE, Level.WARNING, Level.INFO, Level.CONFIG,
