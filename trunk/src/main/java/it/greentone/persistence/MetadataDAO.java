...
public class MetadataDAO extends JdoDaoSupport {
    ...
    public Metadata loadMetadata(final long id) throws DataAccessException {
        final Metadata metadata = getJdoTemplate().getObjectById(Metadata.class, Long.valueOf(id));
        if (metadata == null)
     ...
     * @throws DataAccessException
     */
    public void deleteMetadata(final JobCategory metadata) throws DataAccessException {
    ...
    public Collection<Metadata> getAllMetadata() throws DataAccessException {
        return getPersistenceManager().detachCopyAll(getJdoTemplate().find(Metadata.class));
    }
    ...
    public Metadata getMetadataFromName(String name) {
        ...
        Collection<Metadata> result = getPersistenceManager().detachCopyAll((Collection<Metadata>) query.execute(name));
