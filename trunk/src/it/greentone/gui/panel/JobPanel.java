...
public class JobPanel extends AbstractPanel {
	...
	public JobPanel(ModelEventManager modelEventManager) {
		...
		operationsColumnsNames = new String[] { getResourceMap().getString(LOCALIZATION_PREFIX + "Table.description"), getResourceMap().getString(LOCALIZATION_PREFIX + "Table.date"), getResourceMap().getString(LOCALIZATION_PREFIX + "Table.amount") };
		...
		documentsColumnsNames = new String[] { getResourceMap().getString(LOCALIZATION_PREFIX + "Table.isDigital"), getResourceMap().getString(LOCALIZATION_PREFIX + "Table.protocol"), getResourceMap().getString(LOCALIZATION_PREFIX + "Table.description"),
			...
			documentsTable.setDefaultRenderer(Boolean.class, new TableCellRenderer() {
				...
				public java.awt.Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
					if (value instanceof Boolean) {
						...
						if (v) {
							return new JLabel(getResourceMap().getImageIcon(LOCALIZATION_PREFIX + "Table.isDigitalIcon"));
						} else {
							return new JLabel("");
						}
					}
	...
	/**
	 * Restituisce il pulsante che permette di passare dalla vista a split pane
	 * al dettaglio del documento selezionato.
	 * 
		...
		if (documentDetailButton == null) {
			...
			documentDetailButton.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					int selectedRow = getDocumentsTable().getSelectedRow();
					if (selectedRow > -1) {
						int rowIndexToModel = getDocumentsTable().convertRowIndexToModel(getDocumentsTable().getSelectedRow());
	...
	/**
	 * Restituisce il pulsante che permette di passare dalla vista a split pane
	 * al dettaglio dell'operazione selezionata.
	 * 
	 * @return il pulsante che permette di passare dalla vista a split pane al
		...
		if (operationDetailButton == null) {
			...
			operationDetailButton.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					int selectedRow = getOperationsTable().getSelectedRow();
					if (selectedRow > -1) {
						int rowIndexToModel = getOperationsTable().convertRowIndexToModel(getOperationsTable().getSelectedRow());
						JPanel operationPanel = createOperationPanel(operationsTableModel.getElementAt(rowIndexToModel));
	...
	/**
	 * Restituisce il pulsante per tornare alla vista con split pane della
	 * scheda.
	 * 
	...
	protected JPanel createDocumentPanel(Document document) {
		...
		documentDetailPanel.add(new JLabel(GreenToneUtilities.formatDateTime(document.getReleaseDate())), "growx, wrap");
		...
		documentDetailPanel.add(new JLabel(document.getRecipient() != null ? document.getRecipient().toString() : ""), "growx, wrap");
	...
	protected JPanel createOperationPanel(Operation operation) {
		...
		operationDetailPanel.add(new JLabel(getResourceMap().getString("viewOperations.Panel.isVacazione")), "gap para");
		if (operation.getIsVacazione()) {
			...
			if (operation.getIsProfessionalVacazione()) {
				operationDetailPanel.add(new JLabel(getResourceMap().getString("viewOperations.Panel.isProfessionalVacazione")), "gap para");
				operationDetailPanel.add(new JLabel(getResourceMap().getString(LOCALIZATION_PREFIX + "yes")), "growx, wrap");
			}
		} else {
