...
public class OptionsPanel extends ContextualPanel<Void> {
	...
	private JPanel getSystemPanel() {
		if (systemPanel == null) {
			...
			systemPanel.setBorder(BorderFactory.createTitledBorder(getResourceMap().getString("viewOptions.Panel.systemTitle")));
	...
	private JPanel getAppPanel() {
		if (appPanel == null) {
			...
			appPanel.setBorder(BorderFactory.createTitledBorder(getResourceMap().getString("viewOptions.Panel.appTitle")));
			JLabel vacazioneLabel = new JLabel(getResourceMap().getString("viewOptions.Panel.vacazione"));
			JLabel vacazioneAiutanteLabel = new JLabel(getResourceMap().getString("viewOptions.Panel.vacazioneAiutante"));
			JLabel useYearInJobsProtocolLabel = new JLabel(getResourceMap().getString("viewOptions.Panel.useYearInJobsProtocol"));
	...
	public JFormattedTextField getVacazioneTextField() {
		if (vacazioneTextField == null) {
			...
			/*
			 * Issue 39: accettare anche il punto come punto separatore dei
			 * decimali
			 */
	 ...
	 * di un incarico.
	 * 
	 * @return il flag per l'uso dell'anno nella composizione del protocollo di
	 *         un incarico
	 */
		...
		if (deleteLogsButton == null) {
			deleteLogsButton = new JButton(new AbstractAction() {
				...
				public void actionPerformed(ActionEvent arg0) {
					...
					if (result)
						JOptionPane.showMessageDialog(getParent(), getResourceMap().getString("viewOptions.Panel.logsDeleted"), getResourceMap().getString("Application.title"), JOptionPane.INFORMATION_MESSAGE);
					else
						JOptionPane.showMessageDialog(getParent(), getResourceMap().getString("viewOptions.Panel.logsNotDeleted"), getResourceMap().getString("Application.title"), JOptionPane.WARNING_MESSAGE);
				}
			});
